...
int WebServer::_handle_connection(epoll_event &event)
{
	...
	if (ref.request && ref.request->complete == true) {
		...
		BodyRead		body(ref);			//todo body static, multipart, lettura chunked
		CGI_Handler		cgihandler(ref);	//? #VINCE MA SE IL FILE GENERATO DALLA CGI E' TROPPO GRANDE? CHI LA GESTISCE LA CHUNKED PER LA CGI?
